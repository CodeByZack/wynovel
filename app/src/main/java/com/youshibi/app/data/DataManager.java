...
public class DataManager {
    ...
    public Observable<List<Book>> getChannelBooks(long channelId, int page, int size) {
        HashMap<String, Object> hashMap = new HashMap<>();
        hashMap.put("page", page);
        hashMap.put("category", channelId);
        return RequestClient
                .getServerAPI()
                .getChannelBooks(hashMap)
                .compose(rxCache.<List<Book>>transformer("getChannelBooks"+channelId+page+size,new TypeToken<List<Book>>() {
                }.getType(), CacheStrategy.firstRemote()))
                .map(new Func1<CacheResult<List<Book>>, List<Book>>() {
                    @Override
                    public List<Book> call(CacheResult<List<Book>> dataListCacheResult) {
     ...
     * 获取书籍详情
     */
    public Observable<Book> getBookDetail(String bookId) {
        return RequestClient
                .getServerAPI()
